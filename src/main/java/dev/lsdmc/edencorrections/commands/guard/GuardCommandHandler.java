...
public class GuardCommandHandler {
    ...
    public boolean handleCommand(CommandSender sender, String[] args) {
        ...
        switch (sub) {
            ...
            case "pursue": {
                if (args.length < 2) {
                    player.sendMessage(MessageUtils.getPrefix(plugin).append(MessageUtils.parseMessage("<red>Usage: /g " + sub + " <player></red>")));
                    return true;
                }
            ...
            case "stop": {
                if (args.length < 2) {
                    player.sendMessage(MessageUtils.getPrefix(plugin).append(MessageUtils.parseMessage("<red>Usage: /g endchase <player></red>")));
                    return true;
                }
                Player target = Bukkit.getPlayer(args[1]);
                if (target == null) {
            ...
            case "detain": {
                if (args.length < 2) {
                    player.sendMessage(MessageUtils.getPrefix(plugin).append(MessageUtils.parseMessage("<red>Usage: /g jail <player> [time]</red>")));
                    return true;
                }
                Player target = Bukkit.getPlayer(args[1]);
                if (target == null) {
            ...
            case "jailoffline": {
                if (args.length < 2) {
                    player.sendMessage(MessageUtils.getPrefix(plugin).append(MessageUtils.parseMessage("<red>Usage: /g jailoffline <player></red>")));
                    return true;
                }
                ...
                try {
                    ...
                    player.sendMessage(MessageUtils.getPrefix(plugin).append(
                        MessageUtils.parseMessage("<green>Player will be jailed when they next join the server.</green>")));
                } catch (IllegalArgumentException e) {
                    player.sendMessage(MessageUtils.getPrefix(plugin).append(
                        MessageUtils.parseMessage("<red>Invalid player name/UUID. Player not found in recent records.</red>")));
                }
                return true;
            }
            default:
    ...
    public List<String> onTabComplete(CommandSender sender, String[] args) {
        List<String> subs = List.of("chase", "pursue", "endchase", "stop", "jail", "detain", "jailoffline", "help");
        if (args.length == 1) {
            return subs.stream().filter(s -> s.startsWith(args[0].toLowerCase())).collect(Collectors.toList());
        }
        if (args.length == 2) {
            String sub = args[0].toLowerCase();
            if (subs.contains(sub) && !sub.equals("help")) {
        ...
        if (args.length == 3) {
            String sub = args[0].toLowerCase();
            if (sub.equals("jail") || sub.equals("detain")) {
                // Tab complete jail time options
                return List.of("3", "5", "10", "15", "30").stream()
