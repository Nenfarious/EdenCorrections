...
public class GuardDutyCommandHandler {
    ...
    public List<String> onTabComplete(CommandSender sender, String[] args) {
        ...
        String[] subs = {"duty", "status", "time", "addtime", "settime", "convert", "help"};
        if (args.length == 1) {
            for (String s : subs) {
        ...
        if (args.length == 2) {
            if (List.of("status", "addtime", "settime").contains(args[0].toLowerCase())) {
                return Bukkit.getOnlinePlayers().stream()
                        .map(Player::getName)
                        .filter(name -> name.toLowerCase().startsWith(args[1].toLowerCase()))
                        .collect(Collectors.toList());
            } else if (args[0].equalsIgnoreCase("convert")) {
                return List.of("5", "10", "15", "30", "60", "120", "240", "480").stream()
